...
namespace Assets.Scripts.GraphicCustoms
{
    public class Image
    {
        ...
        public static byte[] loadData(string filename)
        {
            ...
            sbyte[] array = Utils.convertToSbyteArray(textAsset.bytes);
        ...
        private static Image __createImageMini(Image img)
        {
            Image image = new Image();
            Texture2D texture2D = img.texture;
            if (texture2D == null)
            {
            }
            image.texture = texture2D;
        ...
        public static Image __createImage(Image src, int x, int y, int w, int h, int transform)
        {
            Image image = new Image();
