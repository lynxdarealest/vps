...
public class YellowIV extends Boss {
    private static final Logger logger = Logger.getLogger(YellowIV.class);

    public YellowIV(int level) {
        super(BossName.YELLOW_IV, -1);
        typePk = 3;
        this.level = level;
    ...
    public void injure(Entity attacker, long hpInjure, boolean isCritical, boolean isStrikeBack) {
        ...
        if (isDead() && attacker != null && zone != null) {
            List<Player> playerList = zone.getPlayers(Zone.TYPE_PLAYER);
            for (Player player : playerList) {
                Item item = ItemManager.getInstance().createItem(ItemName.VE_THAM_GIA_DAO_KHO_BAU, 1, true);
                item.setExpiry(3);
    ...
    public boolean isDodge(Entity attacker, int accurate) {
        if (attacker == null || (!attacker.isDisciple() && !attacker.isPlayer())) {
            return true;
        }
        Player player = (Player) attacker;
        if (player.mySkill == null || player.mySkill.getDx() > 200) {
            return true;
        }
        return super.isDodge(attacker, accurate);
    }
}
