...
public class Cumber extends Boss {
    ...
    public Cumber(Island island) {
        super(BossName.CUMBER, -1);
        this.island = island;
        typePk = 0;
    ...
    public List<ItemMap> throwItem(Entity killer) {
        ...
        if (Utils.isPercent(5)) {
            ItemMap itemMap = ItemManager.getInstance().createItemMap(ItemName.CAI_TRANG_CUMBER, 1, killer.id);
            itemMap.options.add(new ItemOption(25, Utils.nextInt(10, 15)));
            itemMap.options.add(new ItemOption(31, Utils.nextInt(10, 15)));
            itemMap.options.add(new ItemOption(32, Utils.nextInt(10, 15)));
    ...
    public void startDie(Entity killer) {
        ...
        Utils.setTimeout(() -> {
            Boss boss = new SuperCumber(island);
            boss.joinClient();
