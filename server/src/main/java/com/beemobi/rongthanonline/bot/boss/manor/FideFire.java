...
public class FideFire extends Boss {
    private static final Logger logger = Logger.getLogger(FideFire.class);

    public FideFire(int level) {
        super(BossName.FIDE_FIRE, level);
    ...
    public void updateEveryOneMinutes(long now) {
        ...
        for (Player player : playerList) {
            if (!player.isDead()) {
                ...
                if (!player.isDead()) {
                    player.addEffect(new Effect(player, EffectName.THIEU_DOT, 10000, 1000, 5));
                }
            }
        }
    }
