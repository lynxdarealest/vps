package com.beemobi.rongthanonline.top.player.week.event.previous;
...
public class TopEventPreviousWeekly extends TopWeekly {

    public TopEventPreviousWeekly() {
        super(TopType.TOP_EVENT_PREVIOUS_WEEKLY, "Sự kiện");
        limit = 10;
    }
    ...
    public void init() {
        ...
        try {
            ...
            List<PointWeeklyData> objects = GameRepository.getInstance().pointWeekly.findTopEvent(Server.ID, startOfPreviousWeek, limit);
            for (PointWeeklyData object : objects) {
                TopEventPreviousWeeklyInfo top = new TopEventPreviousWeeklyInfo(null);
    ...
    public void reward(LocalDate week) {
        if (!GameRepository.getInstance().pointWeekly
                .findByServerAndPlayerIdAndStartWeekAndEventPoint(Server.ID, -1, week, -1L).isEmpty()) {
           return;
        }
        PointWeeklyData data = new PointWeeklyData(week);
        data.eventPoint = -1L;
        data.eventTime = new Timestamp(System.currentTimeMillis());
