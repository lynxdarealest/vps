...
import com.beemobi.rongthanonline.top.player.event.TopEventInfo;
...
public class TopEventOther extends TopPlayer {
    private static final Logger logger = Logger.getLogger(TopEventOther.class);

    public TopEventOther() {
        super(TopType.TOP_EVENT_OTHER, "Sự kiện");
        limit = 10;
    }

    @Override
    public void init() {
        List<Object[]> objects = GameRepository.getInstance().playerData.findTopEventOther(Server.ID, limit);
        for (Object[] object : objects) {
            TopEventOtherInfo top = new TopEventOtherInfo(null);
            top.id = (int) object[0];
    ...
    public void setObject(Object object) {
        ...
        try {
            ...
            elements.add(new TopEventOtherInfo(player));
        } finally {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == 0) {
                ...
                if (info.score >= 10000) {
                    ...
                    item.setExpiry(7);
                    item.options.add(new ItemOption(19, 16));
                ...
                if (info.score >= 7500) {
                    Item item = ItemManager.getInstance().createItem(ItemName.CAPSULE_HUYEN_BI, 1, false);
                    item.setExpiry(7);
                    item.options.add(new ItemOption(19, 14));
                    item.options.add(new ItemOption(67, 4));
                    item.options.add(new ItemOption(68, 4));
                    itemList.add(item);
