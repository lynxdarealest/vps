...
public class GhepCaiTrang extends Upgrade {
    private static final Logger logger = Logger.getLogger(GhepCaiTrang.class);

    public GhepCaiTrang(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn các Cải trang muốn ghép");
        notes.add("- Sau đó chọn Ghép");
        notes.add("(!) Lưu ý:");
        notes.add("- Cải trang yêu cầu không có hạn sử dụng");
        notes.add("- Tất cả chỉ số của Cải trang sẽ được cộng dồn vào Cải trang bỏ vào đầu tiên");
        notes.add("- Cải trang sau khi ghép vẫn có thể tách ngược lại");
        notes.add("- Số sao của Cải trang sau khi ép sẽ theo trang bị có nhiều sao nhất");
        notes.add("- Chỉ được chọn các Cải trang chưa được ghép");
    ...
    public boolean isValidItem(Item item, Player player) {
        return item != null && ((item.template.type == ItemType.TYPE_AVATAR && (item.template.gender == player.gender || item.template.gender == -1) && item.getExpiry() == -1
