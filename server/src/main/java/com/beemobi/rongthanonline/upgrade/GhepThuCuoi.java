...
public class GhepThuCuoi extends Upgrade {
    private static final Logger logger = Logger.getLogger(GhepThuCuoi.class);

    public GhepThuCuoi(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn các Thú cưỡi muốn ghép");
        notes.add("- Sau đó chọn Ghép");
        notes.add("(!) Lưu ý:");
        notes.add("- Thú cưỡi yêu cầu không có hạn sử dụng");
        notes.add("- Tất cả chỉ số của Thú cưỡi sẽ được cộng dồn vào Thú cưỡi bỏ vào đầu tiên");
        notes.add("- Thú cưỡi sau khi ghép vẫn có thể tách ngược lại");
        notes.add("- Số sao của Thú cưỡi sau khi ép sẽ theo trang bị có nhiều sao nhất");
        notes.add("- Chỉ được chọn các Thú cưỡi chưa được ghép");
    ...
    public boolean isValidItem(Item item, Player player) {
        return item != null && ((item.template.type == ItemType.TYPE_THU_CUOI && (item.template.gender == player.gender || item.template.gender == -1) && item.getExpiry() == -1
