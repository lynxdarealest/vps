...
public class TachHuyHieu extends Upgrade {
    private static final Logger logger = Logger.getLogger(GhepHuyHieu.class);

    public TachHuyHieu(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn huy hiệu muốn tách");
        notes.add("- Sau đó chọn Tách");
        notes.add("(!) Lưu ý: chỉ có thể tách huy hiệu đã ghép");
    }
    ...
    public boolean isValidItem(Item item) {
        return item != null && item.template.type == ItemType.TYPE_MEDAL && item.getExpiry() == -1 && item.options.stream().anyMatch(o -> o.template.id == 176);
    }
}
