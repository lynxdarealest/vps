...
public class GhepHuyHieu extends Upgrade {
    private static final Logger logger = Logger.getLogger(GhepHuyHieu.class);

    public GhepHuyHieu(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn các huy hiệu muốn ghép");
        notes.add("- Sau đó chọn Ghép");
        notes.add("(!) Lưu ý:");
        notes.add("- Huy hiệu yêu cầu không có hạn sử dụng");
        notes.add("- Tất cả chỉ số của huy hiệu sẽ được cộng dồn vào huy hiệu bỏ vào đầu tiên");
        notes.add("- Huy hiệu sau khi ghép vẫn có thể tách ngược lại");
        notes.add("- Số sao của huy hiệu sau khi ép sẽ theo trang bị có nhiều sao nhất");
        notes.add("- Chỉ được chọn các huy hiệu chưa được ghép");
    ...
    public boolean isValidItem(Item item, Player player) {
        return item != null && ((item.template.type == ItemType.TYPE_MEDAL && (item.template.gender == player.gender || item.template.gender == -1) && item.getExpiry() == -1
