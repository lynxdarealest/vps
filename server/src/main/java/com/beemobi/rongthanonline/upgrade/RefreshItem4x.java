...
public class RefreshItem4x extends Upgrade {
    private static final Logger logger = Logger.getLogger(RefreshItem4x.class);
    private static final int COIN = 2000000;
    private static final int DIAMOND = 25;
    public static HashMap<Integer, Integer> ITEM_REFRESH;

    static {
        ITEM_REFRESH = new HashMap<>();
        ITEM_REFRESH.put(ItemName.GANG_SIEU_NHAN, ItemName.MANH_GANG_4X);
        ITEM_REFRESH.put(ItemName.GIAY_SIEU_NHAN, ItemName.MANH_GIAY_4X);
        ITEM_REFRESH.put(ItemName.QUAN_SIEU_NHAN, ItemName.MANH_QUAN_4X);
        ITEM_REFRESH.put(ItemName.AO_SIEU_NHAN, ItemName.MANH_AO_4X);
        ITEM_REFRESH.put(ItemName.GANG_SIEU_NAMEK, ItemName.MANH_GANG_4X);
        ITEM_REFRESH.put(ItemName.GIAY_SIEU_NAMEK, ItemName.MANH_GIAY_4X);
        ITEM_REFRESH.put(ItemName.QUAN_SIEU_NAMEK, ItemName.MANH_QUAN_4X);
        ITEM_REFRESH.put(ItemName.AO_SIEU_NAMEK, ItemName.MANH_AO_4X);
        ITEM_REFRESH.put(ItemName.GANG_SIEU_SAYAIN, ItemName.MANH_GANG_4X);
        ITEM_REFRESH.put(ItemName.GIAY_SIEU_SAYAIN, ItemName.MANH_GIAY_4X);
        ITEM_REFRESH.put(ItemName.QUAN_SIEU_SAYAIN, ItemName.MANH_QUAN_4X);
        ITEM_REFRESH.put(ItemName.AO_SIEU_SAYAIN, ItemName.MANH_AO_4X);
        ITEM_REFRESH.put(ItemName.NGOC_BOI_SIEU_CAP, ItemName.MANH_BOI_4X);
        ITEM_REFRESH.put(ItemName.NHAN_SIEU_CAP, ItemName.MANH_NHAN_4X);
        ITEM_REFRESH.put(ItemName.DAY_CHUYEN_SIEU_CAP, ItemName.MANH_DAY_CHUYEN_4X);
        ITEM_REFRESH.put(ItemName.RADAR_SIEU_CAP, ItemName.MANH_RADAR_4X);
    }

    public RefreshItem4x(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn trang bị 4x cần tái chế");
        notes.add("- Sau đó chọn mảnh trang bị 4x cùng loại (cần tối thiếu 200 mảnh)");
    ...
    public void upgrade(Message message, Player player) {
        ...
        try {
            if (player.level < 40) {
                player.addInfo(Player.INFO_RED, "Yêu cầu cấp độ từ 40 trở lên");
                return;
            }
            if (player.isBagFull()) {
