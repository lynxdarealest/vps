...
public class TachThuCuoi extends Upgrade {
    private static final Logger logger = Logger.getLogger(TachThuCuoi.class);

    public TachThuCuoi(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn Thú cưỡi muốn tách");
        notes.add("- Sau đó chọn Tách");
        notes.add("(!) Lưu ý: chỉ có thể tách Thú cưỡi đã ghép");
    }
    ...
    public boolean isValidItem(Item item) {
        return item != null && item.template.type == ItemType.TYPE_THU_CUOI && item.getExpiry() == -1 && item.options.stream().anyMatch(o -> o.template.id == 176);
    }
}
