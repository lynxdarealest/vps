...
public class TachAura  extends Upgrade {
    private static final Logger logger = Logger.getLogger(TachAura.class);

    public TachAura(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn Aura muốn tách");
        notes.add("- Sau đó chọn Tách");
        notes.add("(!) Lưu ý: chỉ có thể tách Aura đã ghép");
    }
    ...
    public boolean isValidItem(Item item) {
        return item != null && item.template.type == ItemType.TYPE_AURA && item.getExpiry() == -1 && item.options.stream().anyMatch(o -> o.template.id == 176);
    }
}
