...
public class PackThuCuoi extends Upgrade {
    private static final Logger logger = Logger.getLogger(PackThuCuoi.class);

    private static final int DIAMOND = 200;

    public PackThuCuoi(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn Thú cưỡi cần ghép thêm chỉ số");
        notes.add("- Chọn Thú cưỡi muốn dịch chuyển chỉ số");
        ...
        notes.add("- Sau khi ghép, Thú cưỡi ban đầu sẽ có thêm 1 vài chỉ số của trang bị muốn dịch chuyển");
    ...
    public void upgrade(Message message, Player player) {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (item.template.type == ItemType.TYPE_THU_CUOI &&
            ...
            content.append("Thất bại sẽ không mất Thú cưỡi dịch chuyển").append("\n");
    ...
    public boolean isValidItem(Item item) {
        return item != null && ((item.template.type == ItemType.TYPE_THU_CUOI && item.getExpiry() == -1 && item.options.stream().allMatch(this::isOptionValid))
