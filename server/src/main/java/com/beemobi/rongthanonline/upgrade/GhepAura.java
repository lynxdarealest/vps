...
public class GhepAura extends Upgrade {
    private static final Logger logger = Logger.getLogger(GhepAura.class);

    public GhepAura(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn các Aura muốn ghép");
        notes.add("- Sau đó chọn Ghép");
        notes.add("(!) Lưu ý:");
        notes.add("- Aura yêu cầu không có hạn sử dụng");
        notes.add("- Tất cả chỉ số của Aura sẽ được cộng dồn vào Aura bỏ vào đầu tiên");
        notes.add("- Aura sau khi ghép vẫn có thể tách ngược lại");
        notes.add("- Số sao của Aura sau khi ép sẽ theo trang bị có nhiều sao nhất");
        notes.add("- Chỉ được chọn các Aura chưa được ghép");
    ...
    public boolean isValidItem(Item item, Player player) {
        return item != null && ((item.template.type == ItemType.TYPE_AURA && (item.template.gender == player.gender || item.template.gender == -1) && item.getExpiry() == -1
