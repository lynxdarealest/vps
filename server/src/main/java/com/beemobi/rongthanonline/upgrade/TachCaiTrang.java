...
public class TachCaiTrang extends Upgrade {
    private static final Logger logger = Logger.getLogger(TachCaiTrang.class);

    public TachCaiTrang(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn Cải trang muốn tách");
        notes.add("- Sau đó chọn Tách");
        notes.add("(!) Lưu ý: chỉ có thể tách Cải trang đã ghép");
    }
    ...
    public boolean isValidItem(Item item) {
        return item != null && item.template.type == ItemType.TYPE_AVATAR && item.getExpiry() == -1 && item.options.stream().anyMatch(o -> o.template.id == 176);
    }
}
