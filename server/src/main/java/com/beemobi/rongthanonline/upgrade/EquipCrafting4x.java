...
public class EquipCrafting4x extends Upgrade {
    ...
    private static final int REQUIRE = 999;

    static {
        ITEM_CRAFTING = new HashMap<>();
        ITEM_CRAFTING.put(ItemName.MANH_GANG_4X, new KeyValue<>(20000000, new Integer[]{ItemName.GANG_SIEU_NHAN, ItemName.GANG_SIEU_NAMEK, ItemName.GANG_SIEU_SAYAIN}));
        ITEM_CRAFTING.put(ItemName.MANH_GIAY_4X, new KeyValue<>(12000000, new Integer[]{ItemName.GIAY_SIEU_NHAN, ItemName.GIAY_SIEU_NAMEK, ItemName.GIAY_SIEU_SAYAIN}));
        ITEM_CRAFTING.put(ItemName.MANH_AO_4X, new KeyValue<>(12000000, new Integer[]{ItemName.AO_SIEU_NHAN, ItemName.AO_SIEU_NAMEK, ItemName.AO_SIEU_SAYAIN}));
        ITEM_CRAFTING.put(ItemName.MANH_QUAN_4X, new KeyValue<>(12000000, new Integer[]{ItemName.QUAN_SIEU_NHAN, ItemName.QUAN_SIEU_NAMEK, ItemName.QUAN_SIEU_SAYAIN}));
        ITEM_CRAFTING.put(ItemName.MANH_RADAR_4X, new KeyValue<>(15000000, new Integer[]{ItemName.RADAR_SIEU_CAP, ItemName.RADAR_SIEU_CAP, ItemName.RADAR_SIEU_CAP}));
        ITEM_CRAFTING.put(ItemName.MANH_DAY_CHUYEN_4X, new KeyValue<>(15000000, new Integer[]{ItemName.DAY_CHUYEN_SIEU_CAP, ItemName.DAY_CHUYEN_SIEU_CAP, ItemName.DAY_CHUYEN_SIEU_CAP}));
        ITEM_CRAFTING.put(ItemName.MANH_NHAN_4X, new KeyValue<>(15000000, new Integer[]{ItemName.NHAN_SIEU_CAP, ItemName.NHAN_SIEU_CAP, ItemName.NHAN_SIEU_CAP}));
        ITEM_CRAFTING.put(ItemName.MANH_BOI_4X, new KeyValue<>(15000000, new Integer[]{ItemName.NGOC_BOI_SIEU_CAP, ItemName.NGOC_BOI_SIEU_CAP, ItemName.NGOC_BOI_SIEU_CAP}));
    }

    public EquipCrafting4x(UpgradeType type, String name, String command) {
        super(type, name, command);
        notes.add("- Vào túi đồ chọn Mảnh trang bị 4x");
    ...
    public void confirmUpgrade(Player player, Object[] objects) {
        ...
        try {
            ...
            if (taskMain != null && taskMain.template.id == 17) {
                if (item.template.type == ItemType.TYPE_GANG && taskMain.index == 1) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_BOI && taskMain.index == 2) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_GIAY && taskMain.index == 3) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_NHAN && taskMain.index == 4) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_QUAN && taskMain.index == 5) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_DAY_CHUYEN && taskMain.index == 6) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_AO && taskMain.index == 7) {
                    player.nextTaskIndex();
                } else if (item.template.type == ItemType.TYPE_RADAR && taskMain.index == 8) {
                    player.nextTaskIndex();
                }
            }
        } catch (Exception ex) {
